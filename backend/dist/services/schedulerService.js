...
class SchedulerService {
    ...
    static async processScheduledMessages() {
        ...
        const scheduledMessages = await ScheduledMessage_1.default
        ...
        const results = await Promise.allSettled(scheduledMessages.map(message => this.processSingleMessage(message)));
    ...
    static async processSingleMessage(message) {
        try {
            // Get valid access token for the user
            const accessToken = await slackService_1.SlackService.getValidAccessToken(message.userId.slackUserId);
            // Send message to Slack
            await slackService_1.SlackService.sendMessage(accessToken, message.channelId, message.message);
    ...
    static async bulkUpdateResults(successful, failed) {
        const operations = [];
        ...
        if (operations.length > 0) {
            try {
                await ScheduledMessage_1.default.bulkWrite(operations);
    ...
    static async fallbackIndividualUpdates(successful, failed) {
        console.log('Falling back to individual updates...');
        // Update successful messages individually
        for (const message of successful) {
            try {
                await ScheduledMessage_1.default.updateOne({ _id: message.messageId }, {
                    $set: {
                        sent: true,
